...
def test_wait_for_node_ready_retries_a_transient_wallet_timeout() -> None:
    node = readiness_node()
    synchronized = {"headers": 201, "initialblockdownload": False, "verificationprogress": 1.0}
    node.get_block_count.return_value = 201
    node.get_blockchain_info.return_value = synchronized
    node.ensure_funding_wallet_ready.side_effect = [TimeoutError("createwallet timed out"), None]
    ...
    assert node.ensure_funding_wallet_ready.call_count == 2
    sleep.assert_called_once_with(1)
    wait_for_fee_history_mock.assert_called_once_with(node, 7.0)
